...
public class ReviewPageMaker {
	...
	public static String getPageView(String path, int page,int cNo, int recordPerPage, int totalRecord) {
		...
		if ( beginPageOfBlock < pagePerBlock ) {
			...
			sb.append("<a href='" + path +"?cNo="+cNo +"&page=" + (beginPageOfBlock - 1) + "'>◀</a>&nbsp;&nbsp;");
		}
		...
		for ( int p = beginPageOfBlock; p <= endPageOfBlock; p++ ) {
			if ( p == page ) {
				...
				sb.append("<a href='" + path +"?cNo="+cNo+"&page=" + p + "'>" + p + "</a>&nbsp;&nbsp;");
			}
		}
		...
		if ( endPageOfBlock == totalPage ) {
			...
			sb.append("<a href='" + path +"?cNo="+cNo+ "page=" + (endPageOfBlock + 1) + "'>▶</a>");
		}
